...
// 종(Item) → 표시이름/글리프/색/프레임색/테마.
// SortingItemView, SortingProceduralIconProvider, SortingLevelService가 이 단일 카탈로그를 참조한다.
public sealed class SortingItemDef
{
    public SortingItemType type;
    public SortingTheme theme;
    public string displayName;
    public string glyph;       // 1~2자 폴백 글리프 (PNG 없을 때 표시)
    public Color mainColor;    // 아이콘 본체 색
    public Color accentColor;  // 본체 위 액센트(귀, 잎, 바퀴 등) 색
    public Color frameColor;   // 타일 페이스 미세 색감(가독성용)
}
